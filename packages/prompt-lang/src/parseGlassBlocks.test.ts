...
inside assistant
    ...
    expect(parsed[0].tag).toEqual("Assistant");
    expect(parsed[0].content).toEqual(doc);
    expect(
      doc.substring(
        parsed[0].position.start.offset,
        parsed[0].position.end.offset
      )
    ).toEqual(doc);
    expect(parsed[0].child!.content).toEqual("inside assistant");
    expect(
      doc.substring(
        parsed[0].child!.position.start.offset,
        parsed[0].child!.position.end.offset
      )
    ).toEqual("inside assistant");
  });
...
inside user
    ...
    expect(parsed[1].tag).toEqual("User");
    expect(parsed[1].content).toEqual("<User>\ninside user\n</User>");
    expect(
      doc.substring(
        parsed[1].position.start.offset,
        parsed[1].position.end.offset
      )
    ).toEqual("<User>\ninside user\n</User>");
    expect(parsed[1].child!.content).toEqual("inside user");
    expect(
      doc.substring(
        parsed[1].child!.position.start.offset,
        parsed[1].child!.position.end.offset
      )
    ).toEqual("inside user");
  });
...
inside assistant
    ...
    expect(parsed[0].tag).toEqual("Assistant");
    expect(parsed[0].content).toEqual(expected);
    expect(
      doc.substring(
        parsed[0].position.start.offset,
        parsed[0].position.end.offset
...
<User />`;
    ...
    expect(parsed).toHaveLength(2);
    expect(parsed[0].tag).toEqual("Assistant");
    expect(parsed[0].content).toEqual(`<Assistant />`);
    expect(
      doc.substring(
        parsed[0].position.start.offset,
        parsed[0].position.end.offset
    ...
    expect(parsed[0].child!.content).toEqual("");
    expect(
      doc.substring(
        parsed[0].child!.position.start.offset,
        parsed[0].child!.position.end.offset
    ...
    expect(parsed[1].child!.content).toEqual("");
    expect(
      doc.substring(
        parsed[1].child!.position.start.offset,
        parsed[1].child!.position.end.offset
...
inside user
    ...
    expect(parsed[0].child!.content).toEqual("<User>\ninside user\n</User>");
    expect(
      doc.substring(
        parsed[0].child!.position.start.offset,
        parsed[0].child!.position.end.offset
...
doSomething
    ...
    const parsed = parseGlassBlocks(doc);
    expect(parsed).toHaveLength(1); // doesn't parse inner tag
    expect(parsed[0].content).toEqual(doc);
    expect(parsed[0].child!.content).toEqual(
      `inside assistant
<User>
...
`;
    ...
    expect(parsedDoc).toHaveLength(1);
    expect(parsedDoc[0].content).toEqual('const foo = "bar"\n');
  });
...
restOfTheCode()`;
    ...
    const parsedDoc = parseGlassDocument(doc);
    expect(parsedDoc).toHaveLength(3);
    expect(parsedDoc[0].content).toEqual('const foo = "bar"\n\n');
    expect(parsedDoc[1].content).toEqual(`<Assistant>
<User>
hello world
</User>
</Assistant>`);
    expect(parsedDoc[2].content).toEqual("\n\nrestOfTheCode()");
  });
...
hello world
    ...
    const parsedDoc = parseGlassDocument(doc);
    expect(parsedDoc).toHaveLength(3);
    expect(parsedDoc[0].content).toEqual(`---
language: typescript
---
`);
    expect(parsedDoc[1].content).toEqual('const foo = "bar"\n\n');
    expect(parsedDoc[2].content).toEqual(`<Assistant>
<User>
hello world
</User>
</Assistant>`);
