...
import { type ExtensionContext, type FileSystemWatcher, workspace } from 'vscode';
 ...
 */
async function loadExtensionLogic(context: ExtensionContext): Promise<LabsInfo> {
  try {
    // Clear the module cache to force a reload
    if (process.env.NODE_ENV === 'development') {
      const logicPath = path.join(context.extensionPath, 'dist', 'extension-logic.js');
