...
test('handle rename request of variable for opened references', async () => {
  ...
  const result = await serverHandle.sendRenameRequest(uri, { line: 4, character: 3 }, 'renamed');
...
test('handle undefined rename request', async () => {
  const { uri } = await serverHandle.openTextDocument(fixturePath('node16/undefined-props.mdx'), 'mdx');
  const result = await serverHandle.sendRenameRequest(uri, { line: 4, character: 3 }, 'renamed');
...
test('ignore non-existent mdx files', async () => {
  ...
  const result = await serverHandle.sendRenameRequest(uri, { line: 7, character: 15 }, 'renamed');
