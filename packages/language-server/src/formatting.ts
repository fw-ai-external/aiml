    ...
    const blocks = sections.map((s) => {
      ...
      if (s.type === "block") {
        ...
        const blockWithoutChild =
          s.content.substring(
            0,
            s.child!.position.start.offset - s.position.start.offset
          ) +
          "GLASS_INNERBLOCK_SUBSTITUTION" +
          s.content.substring(
            s.child!.position.end.offset - s.position.start.offset
          );
        ...
        const formatted = prettifyTypescript(blockWithoutChild)
          .replace(
            /\s*GLASS_INNERBLOCK_SUBSTITUTION\s*/,
            "\n" + childContent + "\n"
          )
          .trim();
        return {
