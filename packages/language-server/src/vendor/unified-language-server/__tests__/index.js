...
test('`initialize`', async () => {
  ...
  const initializeResponse = await connection.sendRequest(InitializeRequest.type, {
    processId: null,
    rootUri: null,
    capabilities: {},
...
test('`textDocument/didOpen`, `textDocument/didClose` (and diagnostics)', async () => {
  ...
  const openDiagnosticsPromise = createOnNotificationPromise(PublishDiagnosticsNotification.type);
  ...
  const closeDiagnosticsPromise = createOnNotificationPromise(PublishDiagnosticsNotification.type);
...
test('workspace configuration `requireConfig`', async () => {
  ...
  const openDiagnostics = await openDiagnosticsPromise;
  assert.notEqual(openDiagnostics.diagnostics.length, 0, 'should emit diagnostics on `textDocument/didOpen`');
  assert.deepEqual(
    configRequest,
  ...
  const cachedOpenDiagnosticsPromise = createOnNotificationPromise(PublishDiagnosticsNotification.type);
  connection.sendNotification(DidOpenTextDocumentNotification.type, {
    textDocument: { uri, languageId: 'markdown', version: 1, text: '# hi' },
  });
  await cachedOpenDiagnosticsPromise;
  assert.equal(configRequest, undefined, 'should cache workspace configurations');
  ...
  const reopenDiagnosticsPromise = createOnNotificationPromise(PublishDiagnosticsNotification.type);
  connection.sendNotification(DidOpenTextDocumentNotification.type, {
    textDocument: { uri, languageId: 'markdown', version: 1, text: '# hi' },
  });
  ...
  configRequest = undefined;
  const changeConfigurationDiagnosticsPromise = createOnNotificationPromise(PublishDiagnosticsNotification.type);
  requireConfig = true;
  connection.sendNotification(DidChangeConfigurationNotification.type, {
    settings: {},
  });
  const changeConfigurationDiagnostics = await changeConfigurationDiagnosticsPromise;
  assert.deepEqual(
    configRequest,
    { items: [{ scopeUri: uri, section: 'remark' }] },
...
test('global configuration `requireConfig`', async () => {
  ...
  const watchedFileDiagnosticsPromise = createOnNotificationPromise(PublishDiagnosticsNotification.type);
...
test('`textDocument/formatting`', async () => {
  ...
  const resultBad = await connection.sendRequest(DocumentFormattingRequest.type, {
  ...
  const resultGood = await connection.sendRequest(DocumentFormattingRequest.type, {
    textDocument: { uri: new URL('good.md', import.meta.url).href },
    options: { tabSize: 2, insertSpaces: true },
  });
  assert.deepEqual(resultGood, null, 'should format good documents on `textDocument/formatting`');

  const resultUnknown = await connection.sendRequest(DocumentFormattingRequest.type, {
    textDocument: { uri: new URL('unknown.md', import.meta.url).href },
    options: { tabSize: 2, insertSpaces: true },
  });
  assert.deepEqual(resultUnknown, null, 'should ignore unsynchronized documents on `textDocument/formatting`');
  ...
  const resultOutside = await connection.sendRequest(DocumentFormattingRequest.type, {
    textDocument: {
      uri: new URL('../../outside.md', import.meta.url).href,
    },
    options: { tabSize: 2, insertSpaces: true },
  });
  assert.deepEqual(resultOutside, null, 'should ignore documents outside of workspace on `textDocument/formatting`');
});
...
test('`workspace/didChangeWatchedFiles`', async () => {
  ...
  const changeWatchDiagnosticsPromise = createOnNotificationPromise(PublishDiagnosticsNotification.type);
...
test('`textDocument/codeAction` (and diagnostics)', async () => {
  ...
  const closedCodeActions = await connection.sendRequest(CodeActionRequest.type, {
  ...
  assert.equal(closedCodeActions, null, 'should not emit quick fixes for unsynchronized documents');
});
...
test('`initialize` w/ nothing (finds closest `package.json`)', async () => {
  ...
  connection.sendNotification(DidOpenTextDocumentNotification.type, {
    textDocument: {
      uri: new URL('folder-with-package-json/folder/file.md', import.meta.url).href,
      languageId: 'markdown',
      version: 1,
      text: '# hi',
    },
...
test('`initialize` w/ `rootUri`', async () => {
  ...
  assert.deepEqual(openDiagnostics.diagnostics[0].message, fileURLToPath(cwd).slice(0, -1), 'should use `rootUri`');
});
...
test('`workspace/didChangeWorkspaceFolders`', async () => {
  ...
  const openDiagnostics = await openDiagnosticsPromise;
  assert.equal(openDiagnostics.diagnostics[0].message, fileURLToPath(processCwd).slice(0, -1));

  const didAddDiagnosticsPromise = createOnNotificationPromise(PublishDiagnosticsNotification.type);
  ...
  assert.equal(didAddDiagnostics.diagnostics[0].message, fileURLToPath(otherCwd).slice(0, -1));

  const didRemoveDiagnosticsPromise = createOnNotificationPromise(PublishDiagnosticsNotification.type);
  connection.sendNotification(DidChangeWorkspaceFoldersNotification.type, {
    event: { added: [], removed: [{ uri: otherCwd.href, name: '' }] },
  });
  const didRemoveDiagnostics = await didRemoveDiagnosticsPromise;
  assert.equal(didRemoveDiagnostics.diagnostics[0].message, fileURLToPath(processCwd).slice(0, -1));
});
...
function startLanguageServer(serverFilePath, relativeCwd = './') {
  ...
  if (process.argv.includes('--ipc')) {
    ...
    connection = createProtocolConnection(new IPCMessageReader(serverProcess), new IPCMessageWriter(serverProcess));
    connection.onDispose(() => {
      serverProcess.kill();
    });
  } else {
    const serverProcess = spawn('node', [binary, '--stdio'], { cwd });
    connection = createProtocolConnection(serverProcess.stdout, serverProcess.stdin);
  }
