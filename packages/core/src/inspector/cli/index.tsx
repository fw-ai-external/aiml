...
function InspectorTitle({
  ...
  return (
    <Box paddingLeft={1} width="100%">
      <Text>🦊 Workflow.JSX Inspector</Text>
      <Spacer />
      <Text color="grey">
        {!done ? <Spinner /> : ""}
...
function StatusBar() {
  return (
    <Box width="100%" height={1}>
      <Text>
        Left/right arrow keys to browse history, 1=first, 9=last, q to quit.
      </Text>
    </Box>
  );
}
...
function Inspector({
  ...
  showDebugTree: boolean;
}) {
  const [debugTreeSteps, setDebugTreeSteps] = useState([] as string[]);
  const [debugTreeFrameIndex, setDebugTreeFrameIndex] = useState<number | null>(
    null
  );
  ...
  useInput((input: any, key: any) => {
    if (key.rightArrow) {
      setDebugTreeFrameIndex((prevIndex) =>
        prevIndex === null
          ? debugTreeSteps.length - 1
          : Math.min(debugTreeSteps.length - 1, prevIndex + 1)
      );
    }
    if (key.leftArrow) {
      setDebugTreeFrameIndex((prevIndex) =>
        prevIndex === null
          ? debugTreeSteps.length - 2
  ...
  const debugFrameIndexToUse =
    debugTreeFrameIndex === null
      ? debugTreeSteps.length - 1
      : debugTreeFrameIndex;
      ...
      <Box flexDirection="column" height={rows}>
        ...
        <Box flexDirection="row">
          <LiveStream
            ...
            width={showDebugTree ? "50%" : "100%"}
            height={rows - 6}
          />

          {showDebugTree && (
            <DebugTreeStream
              content={debugTreeSteps[debugFrameIndexToUse] ?? ""}
              width="50%"
              height={rows - 6}
            />
          )}
