...
describe('valideCodeStringExpressions - template strings', () => {
  ...
  test.each(templateStringTestCases)('should %s template: %s', (template, expected) => {
    if (expected === false) {
      // strings are error messages
      expect(typeof valideCodeStringExpressions(template, new Set(['input', 'chatHistory']))).toBe('string');
    } else {
      expect(valideCodeStringExpressions(template, new Set(['input', 'chatHistory']))).toBe(true);
    }
  });
});
...
describe('valideCodeStringExpressions - code strings', () => {
  ...
  test.each(codeStringTestCases)('should %s code string: %s', (code, expected) => {
    if (expected === false) {
      // strings are error messages
      expect(typeof valideCodeStringExpressions(code, new Set(['input', 'chatHistory']), false)).toBe('string');
    } else {
      expect(valideCodeStringExpressions(code, new Set(['input', 'chatHistory']), false)).toBe(true);
    }
  });
});
