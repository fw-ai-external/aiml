...
import type { RunEvent, RunStepStreamEvent } from '../types';
...
describe('AsyncIterableStream', () => {
  ...
  describe('Type handling', () => {
      ...
      const events: RunEvent[] = [
        {
          ...
          uuid: '123', // Added missing uuid property
          runId: '1',
          step: '1',
          event: {
            type: 'api-call',
    ...
    it('should handle RunStepStreamEvent type', () => {
      const events: RunStepStreamEvent[] = [
        { type: 'text-delta', delta: 'something', partial: 'something' },
        { type: 'text', text: 'something' },
      ];
      const stream = new ReplayableAsyncIterableStream<RunStepStreamEvent>(events);
      expect(stream).toBeInstanceOf(ReplayableAsyncIterableStream);
    });

    it('should handle mixed types', () => {
      const events: (RunEvent | RunStepStreamEvent | string)[] = [
        {
          type: 'step-complete',
          finishReason: 'stop',
