...
export const Transition = createElementDefinition({
  ...
  onExecutionGraphConstruction(buildContext): ExecutionGraphElement {
    ...
    for (const ch of buildContext.children) {
      ...
      if ('tag' in ch) {
        const childEG = (ch as BaseElement).onExecutionGraphConstruction?.(buildContext.createNewContextForChild(ch));
        if (childEG) {
    ...
    const id = buildContext.attributes.id || (event ? `transition_${event}` : `transition_${uuidv4()}`);
    ...
    if (target) {
      const maybeTargetElement: BaseElement | undefined = buildContext.findElementByKey(
        target,
        buildContext.fullSpec ?? buildContext.spec,
      );

      // Log the result of the search
      console.log(`Search result for '${target}': ${maybeTargetElement ? 'Found' : 'Not found'}`);

      if (!maybeTargetElement) {
        throw new Error(`Transition ${id} has target ${target} which is not found in the workflow`);
      }

      const targetElement = maybeTargetElement;
      if (!targetElement.onExecutionGraphConstruction) {
        throw new Error(`Transition ${id} has target ${target} which does not support execution graph construction`);
      }

      const targetEG = targetElement.onExecutionGraphConstruction(buildContext.createNewContextForChild(targetElement));

      if (!targetEG) {
        throw new Error(`Transition ${id} has target ${target} which is not found`);
      }
      transitionNode.next = [...(transitionNode.next || []), targetEG];
    }

    // store it in the cache
    buildContext.setCachedGraphElement([key, buildContext.attributes.id].filter(Boolean), transitionNode);
