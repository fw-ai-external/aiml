...
class MockContext implements Partial<ActionContext<any>> {
  ...
  constructor(
    ...
    this.input = options.input || new StepValue({ type: 'text', text: 'input text' });
...
describe('TransitionElement', () => {
  ...
  it('should execute transition with matching event', async () => {
    ...
    if (isErrorResult(valueResult)) {
      throw new Error(`Expected object result but got error: ${valueResult.error}`);
    }
  ...
  it('should not transition if condition is false', async () => {
    ...
    ctx = new MockContext({
      event: 'next',
      evaluateCondition: (cond: string) => (cond === 'count < 0' ? false : true), // Will return false for this condition
    });
