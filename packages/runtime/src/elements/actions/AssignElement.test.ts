...
function isObjectResult(result: any): result is { type: 'object'; object: Record<string, any> } {
  return result && typeof result === 'object' && result.type === 'object' && 'object' in result;
}

// Type guard for error result
function isErrorResult(result: any): result is ErrorResult {
  return result && typeof result === 'object' && result.type === 'error' && 'error' in result && 'code' in result;
}
...
describe('AssignElement', () => {
  ...
  it('should assign a value using expr', async () => {
    ...
    const assignElement = Assign.initFromAttributesAndNodes(ctx.context.attributes, []);
  ...
  it('should assign input value when no expr is provided', async () => {
    ...
    const { contextUpdate, result: stepValue, exception } = await assignElement.execute(ctx as any);
  ...
  it("should return error when variable doesn't exist", async () => {
    ...
    const assignElement = Assign.initFromAttributesAndNodes(ctx.context.attributes, []);
    const { result: stepValue, exception } = await assignElement.execute(ctx as any);
