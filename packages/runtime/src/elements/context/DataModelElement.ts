...
import { type DataElementMetadata, ValueType, getDefaultForType, validateValueType } from '@fireworks/shared';
...
export const DataModel = createElementDefinition({
  ...
  async execute(ctx, childrenNodes) {
    try {
      ...
      const dataElements = childrenNodes.filter((child) => child.elementType === 'data');
...
async function initializeDataElement(
  ...
  if (src) {
    try {
      ...
      value = defaultValue !== undefined ? defaultValue : getDefaultForType(type);
    }
  } else if (expr) {
    try {
      ...
      console.error(`Error evaluating expression for data element ${id}:`, error);
      value = defaultValue !== undefined ? defaultValue : getDefaultForType(type);
    }
  } else if (content) {
    try {
    ...
    try {
      ...
      console.error(`Error getting value from request for data element ${id}:`, error);
      value = defaultValue !== undefined ? defaultValue : getDefaultForType(type);
    }
    ...
    console.error(`Either fromRequest must be true or expr/src must be set for data element ${id}`);
...
function validateDataModel(ctx: InstanceType<typeof ElementExecutionContext<DataModelProps>>): void {
  const scopedModel = (ctx as any).scopedDataModel as ScopedDataModel;
