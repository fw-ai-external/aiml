...
export const Workflow = createElementDefinition({
  ...
  onExecutionGraphConstruction(buildContext) {
    ...
    const childElements = buildContext.children
      .map((child) => {
        if (child instanceof BaseElement) {
          return child.onExecutionGraphConstruction?.(buildContext.createNewContextForChild(child));
        }
        return null;
      })
    ...
    const initial = buildContext.attributes.initial
      ? childElements.find((child) => child.id === buildContext.attributes.initial)
      : undefined;
    ...
    if (initial) {
      ...
      initialStep = childElements.find((child) => child.type === 'state' && child.subType !== 'final');
    }

    // now cache the initial step as "initial" keyword too
    buildContext.setCachedGraphElement(['initial', buildContext.elementKey].filter(Boolean), initialStep!);
