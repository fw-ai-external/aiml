...
describe('AIML Parsing Tests', () => {
  describe('Basic AIML Parsing', () => {
    it('should parse just a prompt into a workflow', async () => {
      ...
      const stateElement = result.nodes[0].children!.find((child) => child.type === 'element' && child.tag === 'state');
      ...
      const llmElement = stateElement?.children?.find((child) => child.tag === 'llm');
...
<workflow id="test">
  ...
  <final id="end" />
      ...
      const states = workflow.children?.filter((child) => child.tag === 'state') || [];
      ...
      const textState = states.find((state) =>
        state.children?.some(
          (child) =>
            ...
            child.attributes.instructions.includes('Some text here with ${userInput.message.content}'),
        ),
      );
...
target="end" />
      ...
      const textState = states.find((state) =>
        state.children?.some(
          (child) =>
            child.tag === 'llm' &&
            typeof child.attributes?.instructions === 'string' &&
            child.attributes.instructions.includes('${userInput.message.content.toLowerCase()}'),
        ),
      );
      expect(textState).not.toBeUndefined();
      ...
      const transitionState = states.find((state) =>
        state.children?.some((child) => child.tag === 'transition' && child.attributes?.target === 'end'),
      );
  ...
  <state id="start">
    <ImportedComponent />
      ...
      if (startState) {
        ...
        const importComponent = startState.children?.find((child) => child.tag === 'ImportedComponent');
          ...
          (child) =>
            ...
            child.children?.some(
              (grandchild) =>
                ...
                grandchild.attributes.instructions.includes('Some text here because why not'),
            ),
        ) || [];
...
Third paragraph (should be merged with above due to blank line).
        ...
        (child) =>
          ...
          child.children?.some(
            (grandchild) =>
              grandchild.tag === 'llm' &&
              typeof grandchild.attributes?.instructions === 'string' &&
              grandchild.attributes.instructions.includes('First paragraph with ${expression1}') &&
              grandchild.attributes.instructions.includes('Second paragraph with ${expression2}') &&
      ...
      const llmElement = textState?.children?.find((child) => child.tag === 'llm');
      expect(llmElement).not.toBeUndefined();

      // Test that all parts of the merged paragraphs are present
      if (llmElement && typeof llmElement.attributes?.instructions === 'string') {
        ...
        expect(instructions).toContain('Third paragraph (should be merged with above due to blank line)');
      }
    });
  });
});
