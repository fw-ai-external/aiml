...
describe('ElementBuilder', () => {
  ...
  describe('parseAttributes', () => {
    test('should parse string attributes', () => {
      ...
      const sourceFile = project.createSourceFile('test.tsx', '<div id="test" class="main" />');
      const element = sourceFile.getFirstDescendantByKind(SyntaxKind.JsxSelfClosingElement);
    ...
    test('should handle boolean attributes', () => {
      ...
      const sourceFile = project.createSourceFile('test.tsx', '<input required disabled />');
      const element = sourceFile.getFirstDescendantByKind(SyntaxKind.JsxSelfClosingElement);
      const attributes = element?.getAttributes() || [];
    ...
    test('should handle JSX expressions', () => {
      ...
      const sourceFile = project.createSourceFile('test.tsx', '<div data={"value"} />');
      const element = sourceFile.getFirstDescendantByKind(SyntaxKind.JsxSelfClosingElement);
      const attributes = element?.getAttributes() || [];
  ...
  describe('createBaseElement', () => {
    test('should create element with basic attributes', () => {
      ...
      const sourceFile = project.createSourceFile('test.tsx', '<State id="test" />');
      const element = sourceFile.getFirstDescendantByKind(SyntaxKind.JsxSelfClosingElement);
    ...
    test('should handle children correctly', () => {
      ...
      const sourceFile = project.createSourceFile('test.tsx', '<State><Action /></State>');
      const element = sourceFile.getFirstDescendantByKind(SyntaxKind.JsxElement);
      if (!element) throw new Error('No element found');
      ...
      const result = ElementBuilder.createBaseElement(element, {}, [childElement]);
