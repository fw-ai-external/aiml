...
<workflow id="test">
  ...
  <state id="second">
    ...
    const transition = firstState?.children?.find(
      (child) =>
        child.tag === "transition" && child.attributes?.target === "second"
    );
...
<workflow id="test">
  ...
  <state id="last">
    ...
    const lastState = workflow.children?.find(
      (child) => child.tag === "state" && child.attributes?.id === "last"
    );
...
<workflow id="test">
  <state id="parent">
    ...
    const transition = nested1?.children?.find(
      (child) =>
        child.tag === "transition" && child.attributes?.target === "nested2"
    );
    expect(transition).not.toBeUndefined();
  });
...
<workflow id="test">
  ...
  <state id="parent2">
    ...
    const parent1 = workflow.children?.find(
      (child) => child.tag === "state" && child.attributes?.id === "parent1"
    );
    expect(parent1).not.toBeUndefined();

    // Find nested state
    const nested = parent1?.children?.find(
      (child) => child.tag === "state" && child.attributes?.id === "nested"
    );
...
<workflow id="test">
  ...
  <state id="next">
    ...
    const grandparent = workflow.children?.find(
      (child) => child.tag === "state" && child.attributes?.id === "grandparent"
    );
    ...
    const child = parent?.children?.find(
      (child) => child.tag === "state" && child.attributes?.id === "child"
    );
    ...
    const transition = child?.children?.find(
      (child) =>
        child.tag === "transition" && child.attributes?.target === "next"
    );
    expect(transition).not.toBeUndefined();
  });
...
<workflow id="test">
  ...
  <state id="state2">
    ...
    const state1 = workflow.children?.find(
      (child) => child.tag === "state" && child.attributes?.id === "state1"
    );
