...
describe('healFlowOrError Phase Tests', () => {
  ...
  function logStateTree(element, indent = 0) {
    ...
    console.log(`${spacing}${element.tag || element.type} ${element.attributes?.id || ''}`);
...
<workflow id="test">
  <state id="start">
    ...
    const finalState = workflow.children?.find((child) => child.type === 'element' && child.tag === 'final');
    expect(finalState).not.toBeUndefined();
    expect(finalState?.attributes?.id).toBe('final');

    // Check for error state
    const errorState = workflow.children?.find(
      (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'error',
    );
...
<workflow id="test">
  ...
  <state id="second">
    ...
    const firstState = workflow.children?.find(
      (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'first',
    );
    ...
    const transition = firstState?.children?.find(
      (child) => child.type === 'element' && child.tag === 'transition' && child.attributes?.target === 'second',
    );
...
<workflow id="test">
  ...
  <state id="last">
    ...
    const lastState = workflow.children?.find(
      (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'last',
    );
    ...
    const transitions = lastState?.children?.filter((child) => child.type === 'element' && child.tag === 'transition');
    expect(transitions?.length).toBeGreaterThan(0);

    // Check if at least one transition targets the final element
    const hasTransitionToFinal = transitions?.some((t) => t.attributes?.target === finalState?.attributes?.id);
...
<workflow id="test">
  <state id="parent">
    ...
    const parentState = workflow.children?.find(
      (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'parent',
    );
    expect(parentState).not.toBeUndefined();

    // Find the first nested state
    const nested1 = parentState?.children?.find(
      (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'nested1',
    );
    expect(nested1).not.toBeUndefined();

    // Check for transition to nested2
    const transition = nested1?.children?.find(
      (child) => child.type === 'element' && child.tag === 'transition' && child.attributes?.target === 'nested2',
    );
    expect(transition).not.toBeUndefined();
  });
...
<workflow id="test">
  ...
  <state id="parent2">
    ...
    const parent1 = workflow.children?.find(
      (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'parent1',
    );
    expect(parent1).not.toBeUndefined();

    // Find nested state
    const nested = parent1?.children?.find(
      (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'nested',
    );
    expect(nested).not.toBeUndefined();

    // Check for transition to parent2
    const transition = nested?.children?.find(
      (child) => child.type === 'element' && child.tag === 'transition' && child.attributes?.target === 'parent2',
    );
    expect(transition).not.toBeUndefined();
  });
...
<workflow id="test">
  ...
  <state id="next">
    ...
    const grandparent = workflow.children?.find(
      (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'grandparent',
    );
    ...
    const child = parent?.children?.find(
      (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'child',
    );
    expect(child).not.toBeUndefined();

    // The child should have a transition that ultimately targets 'next'
    const transition = child?.children?.find(
      (child) => child.type === 'element' && child.tag === 'transition' && child.attributes?.target === 'next',
    );
    expect(transition).not.toBeUndefined();
  });
...
<workflow id="test">
  ...
  <state id="state2">
    ...
    const state1 = workflow.children?.find(
      (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'state1',
    );
    expect(state1).not.toBeUndefined();

    // Check that there's exactly one transition
    const transitions = state1?.children?.filter((child) => child.type === 'element' && child.tag === 'transition');
...
<workflow id="test">
  ...
  <state id="state2">
    ...
    const conditionalTransition = transitions?.find((t) => t.attributes?.cond === 'data.special === true');
    expect(conditionalTransition).not.toBeUndefined();
    expect(conditionalTransition?.attributes?.target).toBe('special');

    const conditionlessTransition = transitions?.find((t) => !t.attributes?.cond);
