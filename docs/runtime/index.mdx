...
# Optional: Install IDE support
npm install -D @aiml/language-server
```
...
The AIML Language Server provides IDE features like syntax highlighting and autocompletion:

```typescript
import { createLanguageServer } from '@aiml/language-server';

const server = createLanguageServer({
    // Custom element definitions for autocompletion
