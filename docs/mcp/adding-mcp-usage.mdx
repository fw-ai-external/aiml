...
<script language="python"> 
    # Access the weather tool methods
    # Assuming 'tools' is an accessible object/dict and methods are async
    forecast = await tools['weather'].getForecast("New York") 
    
    # Access the translator tool methods (assuming a 'translator' tool is defined)
    # translation = await ctx.tools['translator'].translate("Hello world", "es")
...
1.  **API Integration**: Using a tool to fetch data from an external API and store it in the datamodel.
    ...
    ```mdx
    <script lang="python">
      # Assuming ctx and state are passed as dicts, and state is mutable
      async def run(ctx, state): 
          customer_id = state['params']['id'] # Nested dict access
          state['customer'] = await ctx['tools']['crm'].getCustomer(customer_id)
          # No explicit return needed if modifying state directly
    </script>
    ```
    </Tab>
    </Tabs>
...
<script lang="python">
  ...
  # Assuming state is a mutable dict and tools provides async methods
  try:
      state['weatherData'] = await tools.weather.getForecast(state['location']) 
  except Exception as error:
      print(f"Weather tool failed: {error}", file=sys.stderr)
      # Ensure 'errors' key exists before updating
      if 'errors' not in state:
          state['errors'] = {}
      state['errors']['weather'] = f"Failed to fetch weather: {error}"
</script>
```
</Tab>
</Tabs>
