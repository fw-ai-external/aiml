...
function printGraph(node: any, indent = 0) {
  ...
  console.log(`${spaces}ID: ${node.id || node.attributes?.id}, Type: ${node.type}, Tag: ${node.tag || node.subType}`);
...
async function testFinalStateAddition() {
  ...
  const finalElement = workflow.children?.find((child) => child.type === 'element' && child.tag === 'final');

  if (finalElement) {
    console.log(`✅ Final element was automatically added: ${finalElement.attributes?.id}`);
  } else {
  ...
  const startState = workflow.children?.find(
    (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'start',
  );

  const anotherState = workflow.children?.find(
    (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'another_state',
  );

  const thirdState = workflow.children?.find(
    (child) => child.type === 'element' && child.tag === 'state' && child.attributes?.id === 'third_state',
  );
  ...
  console.log('1. Parser automatically adds a <final> element if one does not exist');
  console.log(
  ...
  console.log('- Start state: Should automatically transition to final (leaf node)');
  console.log('- Another state: Should NOT transition to final (has explicit transition to third_state)');
  console.log('- Third state: Should automatically transition to final (leaf node)');
}
