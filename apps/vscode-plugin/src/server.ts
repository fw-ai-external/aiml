...
import { buildActiveToken, getOwnerTagName, TokenType } from "./token";

import { TextDocument } from "vscode-languageserver-textdocument";
import { allElementConfigs } from "@workflow/element-types";
import { z } from "zod";

// Define element types with Zod schemas
type ElementType = {
...
connection.onHover((e) => {
  ...
  if (doc) {
    ...
    if (
      token &&
      token.token &&
      (token.token.type == TokenType.TagName ||
        token.token.type == TokenType.AttributeName)
    ) {
      let content = doc.getText();
      // try find tag element
      if (token.token.type == TokenType.TagName) {
        var tagName = content.substring(
          token.token.startIndex,
        ...
        if (tagName in allElementConfigs) {
          var tag = allElementConfigs[tagName];
          if (tag && tag.documentation) {
            return {
              contents: tag.documentation,
              range,
            };
          }
        }
      ...
      // tag attribute
        ...
        if (ownerTagName) {
          var attrName = content.substring(
            token.token.startIndex,
            token.token.endIndex
          );
          var range = Range.create(
          ...
          if (
            attrName in allElementConfigs[ownerTagName.type].propsSchema.shape
          ) {
            var attr =
              allElementConfigs[ownerTagName.type].propsSchema.shape[attrName];
