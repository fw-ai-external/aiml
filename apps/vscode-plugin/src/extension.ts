  ...
  client
    ...
    .catch((error) => {
      ...
      outputChannel.appendLine(
        `Failed to start language client: ${errorMessage}`
      );
      ...
      { scheme: "file", language: "aiml" },
      {
        provideHover: async (document, position, token) => {
          ...
          try {
            ...
            if (!hover) {
              outputChannel.appendLine(
                `No hover information available at ${position.line}:${position.character}`
              );
              return null;
            }

            outputChannel.appendLine(
              `Hover information provided at ${position.line}:${position.character}`
            );
            return hover;
          } catch (error) {
            const errorMessage =
              error instanceof Error ? error.message : String(error);
            outputChannel.appendLine(
              `Error in hover provider: ${errorMessage}`
            );
