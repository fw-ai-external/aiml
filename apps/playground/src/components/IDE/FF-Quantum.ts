export const FFQuantumTheme = {
  dark: {
    colors: {
      "editor.background": "#191a1f", // updated from "#181d20"
      "activityBar.activeBorder": "#46afe3",
      "activityBar.background": "#151419", // darkened relative to editor.background
      "activityBar.foreground": "#a9bacb",
      "activityBarBadge.background": "#343c45",
      "activityBarBadge.foreground": "#b6babf",
      "badge.background": "#343c45",
      "button.background": "#252c33",
      "button.foreground": "#b6babf",
      "button.hoverBackground": "#1d4f73",
      "debugToolBar.background": "#191a1f", // updated
      "dropdown.background": "#191a1f", // updated
      "editor.foreground": "#b6babf",
      "editor.lineHighlightBackground": "#1a1b21", // slight variant for subtle highlight
      "editor.rangeHighlightBackground": "#eb5568d9",
      "editor.selectionBackground": "#1d4f73",
      "editor.selectionForeground": "#f5f7fa",
      "editor.wordHighlightBorder": "#252c33",
      "editorBracketHighlight.foreground": "#70bf53",
      "editorBracketHighlight.unexpectedBracket.foreground": "#343c45",
      "editorBracketMatch.background": "#151419", // updated dark variant
      "editorBracketMatch.border": "#a9bacb",
      "editorCursor.foreground": "#ffffb4",
      "editorGroupHeader.tabsBackground": "#191a1f", // updated
      "editorIndentGuide.activeBackground": "#252c33aa",
      "editorIndentGuide.background": "#252c33",
      "editorWidget.background": "#191a1f", // updated
      "extensionButton.background": "#252c33",
      "extensionButton.foreground": "#b6babf",
      "extensionButton.hoverBackground": "#1d4f73",
      focusBorder: "#343c45",
      foreground: "#b6babf",
      "icon.foreground": "#b6babf",
      "input.background": "#252c33",
      "list.activeBackground": "#252c33",
      "list.activeSelectionBackground": "#252c33",
      "list.activeSelectionForeground": "#b6babf",
      "list.focusBackground": "#252c33",
      "list.hoverBackground": "#252c33",
      "list.inactiveSelectionBackground": "#191a1f", // updated
      "panel.background": "#191a1f", // updated
      "scrollbarSlider.background": "#252c33aa",
      "sideBar.background": "#151419", // updated (from "#14171a")
      "sideBarSectionHeader.background": "#191a1f", // updated (from "#181d20")
      "sideBarTitle.foreground": "#b6babf",
      "sidebarSectionHeader.foreground": "#b6babf",
      "statusBar.background": "#191a1f", // updated
      "statusBar.debuggingBackground": "#b28025",
      "statusBar.debuggingForeground": "#f5f7fa",
      "statusBar.focusBorder": "#46afe3",
      "statusBar.foreground": "#b6babf",
      "statusBar.noFolderBackground": "#191a1f", // updated
      "statusBar.offlineBackground": "#191a1f", // updated
      "statusBar.offlineForeground": "#b6babf",
      "statusBarItem.errorBackground": "#eb5368",
      "statusBarItem.foreground": "#b6babf",
      "statusBarItem.hoverBackground": "#343c45",
      "statusBarItem.prominentHoverBackground": "#1d4f73",
      "statusBarItem.remoteForeground": "#b6babf",
      "statusBarItem.warningBackground": "#d96629",
      "tab.activeModifiedBorder": "#6b7abb",
      "tab.border": "#151419", // updated (from "#14171a")
      "tab.focusedHoverBackground": "#191a1f", // updated
      "tab.inactiveBackground": "#191a1f", // updated
      "tab.unfocusedHoverBackground": "#151419", // updated (from "#14171a")
      "terminal.ansiBlack": "#151419", // updated (from "#14171a")
      "terminal.ansiBlue": "#46afe3",
      "terminal.ansiBrightRed": "#d96629",
      "terminal.ansiGreen": "#70bf53",
      "terminal.ansiRed": "#eb5368",
      "terminal.ansiWhite": "#b6babf",
      "terminal.ansiYellow": "#ffffb4",
      "titleBar.activeBackground": "#151419", // updated (from "#14171a")
      "titleBar.activeForeground": "#b6babf",
      "titleBar.inactiveBackground": "#191a1f", // updated
    },
  },
  tokenColors: [
    {
      name: "highlight_blue",
      scope: [
        "beginning.punctuation.definition.list.markdown",
        "beginning.punctuation.definition.quote.markdown",
        "constant.character.escape",
        "constant.keyword.clojure",
        "entity.name.tag",
        "entity.name.tag.xml",
        "entity.name.variable.enum-member.cs",
        "entity.name.variable.event.cs",
        "entity.name.variable.field.cs",
        "entity.other.attribute-name.class",
        "keyword.control.css",
        "keyword.control.less",
        "keyword.control.sass",
        "keyword.control.scss",
        "keyword.control.stylus",
        "markup.deleted.git_gutter",
        "markup.heading.markdown punctuation.definition.heading.markdown",
        "meta.scope.prerequisites",
        "meta.tag.sgml",
        "meta.tag.xml",
        "punctuation.definition.keyword.css",
        "punctuation.definition.keyword.less",
        "punctuation.definition.keyword.sass",
        "punctuation.definition.keyword.scss",
        "punctuation.definition.keyword.stylus",
        "punctuation.definition.list.begin.markdown",
        "punctuation.definition.metadata.markdown",
        "punctuation.definition.string.markdown",
        "punctuation.section.embedded.begin",
        "punctuation.section.embedded.end",
        "source.sass keyword.control",
        "string.regexp",
        "support.variable",
        "text.variable",
        "token.info-token",
        "variable",
        "variable.other",
        "variable.parameter",
      ],
      settings: {
        foreground: "#46afe3",
      },
    },
    {
      name: "highlight_pink",
      scope: [
        "constant.language",
        "constant.language.boolean",
        "constant.language.json",
        "constant.other.bareword",
        "entity.name",
        "entity.other.attribute-name",
        "entity.other.attribute-name.xml",
        "entity.other.attribute-name.class.mixin",
        "entity.other.attribute-name.id.css",
        "keyword",
        "keyword.other.class.cs",
        "keyword.other.delegate.cs",
        "keyword.other.enum.cs",
        "keyword.other.event.cs",
        "keyword.other.fn.rust",
        "keyword.other.get.cs",
        "keyword.other.julia",
        "keyword.other.namespace.cs",
        "keyword.other.new.cs",
        "keyword.other.set.cs",
        "keyword.other.sql",
        "keyword.other.struct.cs",
        "markup.changed.git_gutter",
        "meta.use.php",
        "source.css support.constant.property-value",
        "source.less support.constant.property-value",
        "source.postcss support.constant.property-value",
        "source.sass support.constant.property-value",
        "source.scss support.constant.property-value",
        "source.stylus support.constant.property-value",
        "storage.control",
        "storage.modifier",
        "storage.type",
        "support.class",
        "support.constant.color",
        "support.function.misc.css",
        "support.function.misc.less",
        "support.function.misc.sass",
        "support.function.misc.scss",
        "support.function.misc.stylus",
        "support.other.namespace.php",
        "support.other.namespace.use.php",
        "support.type",
        "support.type.sys-types",
        "support.type.vendored.property-name",
        "text.html.basic entity.other.attribute-name",
        "text.html.basic entity.other.attribute-name.html",
        "text.xml entity.other.attribute-name",
        "variable.language",
      ],
      settings: {
        foreground: "#df80ff",
      },
    },
    {
      name: "highlight_purple",
      scope: [
        "constant.numeric",
        "constant.other.key",
        "constant.other.symbol",
        "entity.global.clojure",
        "entity.name.module.js",
        "entity.other.inherited-class",
        "markup.heading",
        "markup.inserted.git_gutter",
        "meta.group.braces.curly constant.other.object.key.js string.unquoted.label.js",
        "string",
        "string.quoted.double.xml",
        "string.quoted.single.xml",
        "variable.import.parameter.js",
        "variable.other.class.js",
      ],
      settings: {
        foreground: "#6b7abb",
      },
    },
    {
      name: "highlight_red",
      scope: [
        "constant.other.color",
        "keyword.other.substitution",
        "keyword.other.template",
        "markup.deleted",
        "string.other.link",
        "support.other.variable",
        "variable.css",
        "variable.postcss",
        "variable.sass",
        "variable.scss",
        "variable.stylus",
      ],
      settings: {
        foreground: "#eb5368",
      },
    },
    {
      name: "highlight_orange",
      scope: [
        "invalid",
        "invalid.illegal",
        "keyword.other.DML.sql",
        "keyword.symbol.fsharp",
        "markup.changed",
        "meta.symbol.clojure",
        "punctuation.definition.interpolation",
        "punctuation.definition.template-expression",
        "punctuation.section.bracket.curly.variable.begin.shell.dockerfile",
        "punctuation.section.bracket.curly.variable.end.shell.dockerfile",
        "punctuation.section.embedded.begin.php",
        "punctuation.section.embedded.coffee",
        "punctuation.section.embedded.end.php",
        "support.function.macro.julia",
        "token.error-token",
      ],
      settings: {
        foreground: "#d96629",
      },
    },
    {
      name: "highlight_lightorange",
      scope: [
        "markup.fenced_code.block.markdown",
        "markup.inline.raw.string.markdown",
        "markup.raw.block",
        "token.warn-token",
      ],
      settings: {
        foreground: "#d99b28",
      },
    },
    {
      name: "highlight_green",
      scope: [
        "constant.character",
        "constant.escape",
        "constant.other.timestamp.yaml",
        "entity.name.function",
        "entity.name.section.fsharp",
        "keyword.other",
        "keyword.other.special-method",
        "keyword.other.unit",
        "log.exceptiontype",
        "markup.inserted",
        "meta.class-method.js entity.name.function.js",
        "meta.function-call.generic",
        "meta.method.groovy",
        "meta.object-literal.key",
        "meta.symbol",
        "source.css support.type.property-name",
        "source.less support.type.property-name",
        "source.postcss support.type.property-name",
        "source.sass support.type.property-name",
        "source.scss support.type.property-name",
        "source.stylus support.type.property-name",
        "support.constant",
        "support.constant.media",
        "support.function",
        "support.type.property-name.json",
        "support.variable.property",
        "variable.function",
        "variable.function.constructor",
        "variable.other.property",
      ],
      settings: {
        foreground: "#70bf53",
      },
    },
    {
      name: "highlight_bluegrey",
      scope: [
        "keyword.other.unit",
        "source.css support.constant",
        "source.less support.constant",
        "source.postcss support.constant",
        "source.sass support.constant",
        "source.scss support.constant",
        "source.stylus support.constant",
        "text.bracketed",
      ],
      settings: {
        foreground: "#5e88b0",
      },
    },
    {
      name: "highlight_yellow",
      scope: [
        "entity.name.namespace.rust",
        "entity.name.type.anchor.yaml ",
        "entity.name.type.delegate.cs",
        "entity.name.type.enum.cs",
        "entity.name.type.fsharp",
        "entity.name.type.namespace.cs",
        "entity.other.attribute-name.pseudo-class.css",
        "entity.other.attribute-name.pseudo-element.css",
        "keyword.operator.namespace.rust",
        "keyword.other.type.php",
        "keyword.type.cs",
        "storage.type.annotation",
        "storage.type.cs",
        "storage.type.tag-handle.yaml",
        "support.class.dart",
        "support.type",
      ],
      settings: {
        foreground: "#ffffb4",
      },
    },
    {
      name: "Italic Markup",
      scope: ["markup.italic"],
      settings: {
        foreground: "#eb5368",
        fontStyle: "italic",
      },
    },
    {
      name: "Bold Markup",
      scope: ["markup.bold", "markup.bold string"],
      settings: {
        foreground: "#eb5368",
        fontStyle: "bold",
      },
    },
    {
      name: "Underline Markup",
      scope: ["markup.underline"],
      settings: {
        foreground: "#eb5368",
        fontStyle: "underline",
      },
    },
    {
      name: "Strikethrough Markup",
      scope: ["markup.strike"],
      settings: {
        foreground: "#eb5368",
        fontStyle: "strike",
      },
    },
    {
      name: "Link Markup",
      scope: [
        "constant.other.reference.link.markdown",
        "string.other.link.description.title.markdown",
        "string.other.link.title.markdown",
      ],
      settings: {
        foreground: "#eb5368",
      },
    },
    {
      name: "Markup Quotes",
      scope: ["markup.quote"],
      settings: {
        foreground: "#5c6773",
        fontStyle: "italic",
      },
    },
    {
      name: "Comments",
      scope: ["comment", "punctuation.definition.comment"],
      settings: {
        foreground: "#5c6773",
      },
    },
    {
      name: "Markup Separator",
      scope: ["meta.separator"],
      settings: {
        foreground: "#6b7abb",
        background: "#00000050",
        fontStyle: "bold",
      },
    },
    {
      name: "Nix configuration",
      scope: ["entity.other.attribute-name.multipart.nix"],
      settings: {
        foreground: "#a9bacb",
      },
    },
    {
      name: "ES7 Bind Operator",
      scope: [
        "source.js constant.other.object.key.js string.unquoted.label.js",
      ],
      settings: {
        foreground: "#6b7abb",
      },
    },
  ],
};
