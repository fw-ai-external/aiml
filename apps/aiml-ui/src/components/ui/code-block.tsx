...
function CodeBlockDemo({
  ...
  return (
    <CodeBlock code={code} language={language} theme={themes.oneDark}>
      {filename ? (
        <div className="absolute w-full px-6 py-2 pl-4 text-sm rounded bg-aiml-bg-2 text-aiml-el-6/50">
          {filename}
        </div>
      ) : null}
      ...
      <CodeBlock.Code
        className={cn(
          "bg-transparent h-full p-6 rounded-xl whitespace-pre-wrap",
          filename ? "pt-10" : "",
          className
        )}
      >
        <div className="table-row">
          <div className="flex items-center">
            ...
            <CodeBlock.LineContent className="flex">
              <CodeBlock.Token className="font-mono text-sm aiml-token" />
