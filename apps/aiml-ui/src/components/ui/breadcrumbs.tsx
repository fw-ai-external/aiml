...
const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<'ol'>>(
  ({ className, ...props }, ref) => (
    <ol
      ref={ref}
      className={cn(
        'text-aiml-el-6 flex items-center gap-2 break-words text-[calc(13_/_16_*_1rem)] font-medium',
        className,
      )}
      {...props}
    />
...
const BreadcrumbPage = React.forwardRef<HTMLSpanElement, React.ComponentPropsWithoutRef<'span'>>(
  ({ className, ...props }, ref) => (
    <span
      ...
      aria-disabled="true"
      aria-current="page"
      className={cn('text-aiml-el-5 text-sm font-medium', className)}
      {...props}
    />
  ),
);
...
const BreadcrumbSeparator = ({ children, className, ...props }: React.ComponentProps<'li'>) => (
  <li role="presentation" aria-hidden="true" className={cn(className)} {...props}>
    {children ?? <ChevronDown className="text-aiml-el-3 !h-[0.625rem] !w-[0.625rem] -rotate-90" />}
  </li>
);
