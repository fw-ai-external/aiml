...
export function VersionItem({
  ...
  return (
    <div className={`rounded-md border ${isExpanded ? 'border-aiml-purple/30' : 'border-aiml-bg-3'} bg-aiml-bg-2`}>
      <div
        className="p-2 flex items-center justify-between cursor-pointer hover:bg-aimlg-3/50"
        onClick={onToggleExpand}
      >
        <div className="flex items-center space-x-2">
          <ChevronRight className={`h-3 w-3 transition-transform ${isExpanded ? 'rotate-90 text-aimlurple' : ''}`} />
          <div>
            <div className="flex items-center space-x-2">
              <p className="text-xs font-medium text-aimll-4">Version {index + 1}</p>
            ...
            <p className="text-[10px] text-aimll-3">{version.timestamp.toLocaleString()}</p>
          </div>
        </div>
        <VersionActions
          version={version}
            ...
            {version.evals && version.evals.length > 0 && (
              <div className="space-y-1">
                ...
                >
                  ...
                  <p className="text-xs font-medium text-aimll-4">Evaluations ({version.evals.length})</p>
                </div>
                {showEvals && (
                  <div className="pl-4">
                    <div className="space-y-1 pr-4">
                      {version.evals.map((metric, evalIndex) => (
                        <div key={evalIndex} className="rounded-md border border-aimlg-3 p-1.5 text-[10px]">
                          ...
                          <div className="mt-1 space-y-1">
                            <div className="flex gap-1.5">
                              ...
                              <span className="text-aimll-4">{metric.input}</span>
                            </div>
                            <div className="flex gap-1.5">
                              <span className="text-aimll-3 shrink-0">←</span>
                              <span className="text-aimll-4">{metric.output}</span>
                            </div>
