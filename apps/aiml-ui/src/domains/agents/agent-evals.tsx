...
const AgentEvalsContext = createContext<AgentEvalsContextType>({
  handleRefresh: () => {},
  isLoading: false,
});
...
function EvalTable({
  ...
  return (
    <div className="min-h-0 grid grid-rows-[auto_1fr]">
      <div className="flex items-center gap-4 p-4 bg-aimlg-2 rounded-lg">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-aimll-3" />
          <Input
            id="search-input"
              ...
              {showLoading ? (
                ...
                groupEvals(evals).map((group) => (
                  <React.Fragment key={group.metricName}>
                    <TableRow
                      className="border-b-gray-6 border-b-[0.1px] text-[0.8125rem] cursor-pointer hover:bg-aimlg-3"
                    ...
                    >
                      ...
                      <TableCell className="min-w-[200px] max-w-[30%] font-medium text-aimll-5">
                        {group.metricName}
                      </TableCell>
                      <TableCell className="flex-1 text-aimll-5" />
                      <TableCell className="w-48 text-aimll-5">
