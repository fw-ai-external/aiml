...
export function WorkflowConditionNode({ data }: NodeProps<ConditionNode>) {
  ...
  return (
    <div className={cn('bg-aiml-bg-3 rounded-md min-w-[154.23px] max-w-[274px] flex flex-col p-2 gap-2')}>
            ...
            {condition.ref.step ? (
              <div className="flex items-center gap-1">
                <Text
                  size={'xs'}
                  weight="medium"
                  className="text-aimll-3 bg-aiaiml11 my-auto block rounded-[0.125rem] px-2 py-1 text-[10px]"
                >
                  {condition.conj?.toLocaleUpperCase() || 'WHEN'}
                </Text>

                <Text size={'xs'} className=" text-aimll-3">
